...
class _PinCodePointsState extends State<PinCodePoints>
  ...
  void initState() {
    ...
    animation =
        Tween<double>(
            begin: widget.pinCodeLength.toDouble(),
            end: 0,
          ).animate(controller)
          ..addListener(() async {
            setState(() {});
...
class _Point extends StatelessWidget {
  const _Point({required this.hasError, required this.isFilled});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: context.colors.black100.withValues(
            alpha: isFilled || hasError ? 0.0 : 0.6,
          ),
        ),
      ),
    );
