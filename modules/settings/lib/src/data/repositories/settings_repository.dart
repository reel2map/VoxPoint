...
class SettingsRepositoryImpl implements SettingsRepository {
  ...
  final String _checkPushNotificationsKey = 'check_push_notifications_key';
    ...
    try {
      return Right(
        UserSettingsDTO(
          ...
          checkPushNotifications: _checkPushNotifications(),
        ),
      );
    } catch (error) {
  ...
  Future<Either<Failure, AppVersionEntity>> getStoreVersion() async {
    try {
      return const Right(AppVersionEntity(major: 1, minor: 0, patch: 0));
    } catch (error) {
      return Left(SettingsFailure(code: -1, message: error.toString()));
    }
  }
  ...
  @override
  Future<void> setCheckPushNotifications() async {
    return _sharedStorage.setString(_checkPushNotificationsKey, 'true');
  }
  ...
  bool _checkPushNotifications() {
    final _checkPushNotificationsStr = _sharedStorage.getString(
      _checkPushNotificationsKey,
    );

    return _checkPushNotificationsStr == 'true';
  }
  ...
  Future<Either<Failure, void>> setPushToken({
    ...
    try {
      final response = await _dataSource.setPushToken(deviceId, token);
  ...
  @override
  Future<Either<Failure, void>> setPushSettings(MobilePushConfig config) async {
    try {
      final response = await _dataSource.setMobileSettings(config);
  ...
  Future<Either<Failure, void>> removePushToken(String token) async {
    try {
      final response = await _dataSource.removePushToken(token);
